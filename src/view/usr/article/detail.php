...
<section class="lg:container mx-auto px-4">
    ...
    <nav class="flex h-full">
        <button class="p-2 w-40 bg-blue-900 text-white rounded-lg hover:bg-white hover:text-blue-900"
            onclick="location.href='./list'">글 리스트</button>
        <form action="./modify" method="POST">
            <input type="hidden" name="id" value="<?= $article['id'] ?>">
            <input type="hidden" name="title" value="<?= $article['title'] ?>">
            <input type="hidden" name="body" value="<?= $article['body'] ?>">
            <input class="p-2 mx-2 w-40 bg-blue-900 text-white rounded-lg hover:bg-white hover:text-blue-900" type="submit"
                value="수정">
        </form>
        <form action="./doDelete" method="POST">
            <input type="hidden" name="id" value="<?= $article['id'] ?>">
            <input type="hidden" name="memberId" id="<?= $article['memberId'] ?>">
            <input class="p-2 w-40 bg-blue-900 text-white rounded-lg hover:bg-white hover:text-blue-900" type="submit"
                value="삭제">
        </form>
    </nav>
    ...
    <div>
        ...
        <form class="flex h-full" name="replyForm" action="../reply/doWrite" method="POST">
            ...
            <br>
            <button class="w-1/5 bg-blue-900 text-white rounded-lg" type="submit">작성 완료</button>
            <hr>
        </form>
    </div>
    <div>
        ...
        <nav class="flex h-full">
            <form action="../reply/doModify" method="POST">
                <input class="p-2 w-40 rounded-lg bg-blue-900 text-white hover:bg-white hover:text-blue-900"
                    type="button" value="수정" onclick="toggleText(<?= $i ?>, <?= $reply['memberId'] ?>)">
            </form>
            <form action="../reply/doDelete">
                ...
                <input class="p-2 mx-2 w-40 rounded-lg bg-blue-900 text-white hover:bg-white hover:text-blue-900"
                    type="submit" value="삭제" onclick="if(confirm('정말 삭제 하시겠습니까?') == false) return false;">
            </form>
        </nav>
        <br>
        <form class="flex h-full" action="../reply/doModify" method="POST">
            ...
            <input type="hidden" name="memberId" value="<?= $reply['memberId'] ?>">
            <input type="hidden" name="relId" value="<?= $reply['relId'] ?>">
            <textarea class="w-4/5 p-4" style="display:none" id="modifyReply<?= $i ?>"
                name="body"><?= $reply['body'] ?></textarea>
            <input class="w-1/5 bg-blue-900" type="submit" value="수정 하기" id="modifyBTN<?= $i ?>" style="display:none">
        </form>
        <hr>
        <?php } ?>
    </div>
